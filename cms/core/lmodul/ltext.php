	...
	class LText extends IText{
		public static function parse($content, $flags=static::PARSE_ALL, $options=array()){
			...
			if(bitmask($flags, static::PARSE_INSTALLED)){
				$html=preg_replace_callback('@<!\\-\\-!IsInstalled\\(((.*?)(?:, ?(.*?))?)\\)\\-\\->(.*?)<!\\-\\-/IsInstalled\\(\\1\\)\\-\\->@s', "\\LCMS\\Core\\LText::pyinstall", $html);
				$html=preg_replace_callback('@<!\\-\\-!NotInstalled\\(((.*?)(?:, ?(.*?))?)\\)\\-\\->(.*?)<!\\-\\-/NotInstalled\\(\\1\\)\\-\\->@s', "\\LCMS\\Core\\LText::pninstall", $html);
			}
			if(bitmask($flags, static::PARSE_PART)){
				$html=preg_replace_callback('@<!\\-\\-!PART_(.*?)\\-\\->@', "\\LCMS\\Core\\LText::ppart", $html);
				$html=preg_replace_callback('@<!\\-\\-!THE_(.*?)\\-\\->@', "\\LCMS\\Core\\LText::ppart", $html);
			}
			if(bitmask($flags, static::PARSE_ACTION)){
				$html=preg_replace_callback('@<!\\-\\-!ACTION_(.*?)\\-\\->@', "\\LCMS\\Core\\LText::pact", $html);
			}
			if(bitmask($flags, static::PARSE_LOCALE)){
				$html=preg_replace_callback('@\\-\\-\\-(.*?)\\-\\-\\-@', "\\LCMS\\Core\\LText::plocale", $html);
				$html=preg_replace_callback('@\\+\\+\\+(.*?)\\+\\+\\+@', "\\LCMS\\Core\\LText::plocalew", $html);
			}
